...
$zhen = $_POST["yanzhen"];  //接受系統發送的驗證碼

if ($yan === $zhen) { //驗證碼正確
  header("Content-Type:text/html; charset=utf-8");

  $link = mysql_pconnect("192.168.2.39", "root", "estinet"); 
  ...
  $id = mysql_insert_id();
  $qr_code='http://chart.apis.google.com/chart?cht=qr&choe=UTF-8&chs=300x300&chl=http://'.$_SERVER['HTTP_HOST'].'/update_page.php?id='.$id;
  $update_time = date('Y/m/d H:i:s');

  $query = "INSERT INTO important_info (qr_code,update_time) VALUES ('$qr_code','$update_time')";
  mysql_query($query) or die("無法送出" . mysql_error( ));
